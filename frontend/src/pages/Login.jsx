...
export default function Login() {
    ...
    const handleSubmit = async (e) => {
        ...
        catch (err) { setError(err.response?.data?.message || 'Usuario o contraseña incorrectos') }
    ...
    return (
        ...
        .photo-bg::after { content:''; position:absolute; inset:0; background:radial-gradient(ellipse 55% 60% at 28% 50%,rgba(8,8,9,0.82) 0%,transparent 70%); pointer-events:none; z-index:1; }
        .l-wrap { position:relative; margin-bottom:20px; }
        .l-wrap label { display:block; font-size:11px; font-weight:600; letter-spacing:1.2px; text-transform:uppercase; margin-bottom:8px; transition:color .3s; }
        .l-wrap input { width:100%; padding:13px 16px; border-radius:8px; font-family:'Syne',sans-serif; font-size:14px; outline:none; transition:border-color .3s,box-shadow .3s,transform .2s; }
        .l-wrap input:focus { transform:translateY(-1px); }
        .l-line { position:absolute; bottom:0; left:0; height:2px; background:#c9a84c; border-radius:2px; transition:width .35s cubic-bezier(.22,1,.36,1); }
      ...
      `}</style>
            ...
            <div className="photo-bg" style={{ position:'absolute', inset:0, display:'flex', gap:'10px', padding:'0 10px', zIndex:0, overflow:'hidden' }}>
            ...
            <div style={{ position:'relative', zIndex:2, flex:1, display:'flex', flexDirection:'column', justifyContent:'center', alignItems:'center', padding:'60px 40px' }}>
                ...
                <p style={{ fontSize:15, color: t.dark ? 'rgba(240,239,232,0.65)' : 'rgba(26,20,16,0.7)', textAlign:'center', lineHeight:2, marginTop:24, maxWidth:280, textShadow: t.dark ? '0 1px 12px rgba(0,0,0,0.9)' : 'none' }}>
                    Tu biblioteca visual privada.<br/>Organiza, explora y comparte<br/>fotografías de alta calidad.
                </p>
            </div>
            ...
            <div style={{ position:'relative', zIndex:2, width:480, display:'flex', flexDirection:'column', justifyContent:'center', padding:'60px 52px', background:t.panelBg, backdropFilter:'blur(24px)', borderLeft:`1px solid ${t.panelBorder}`, transition:'background .4s' }}>
                <h1 style={{ fontFamily:'Playfair Display,serif', fontSize:32, fontWeight:700, marginBottom:8, color:t.text }}>Iniciar sesión</h1>
                <p style={{ color:t.muted, fontSize:14, marginBottom:32 }}>Bienvenido de nuevo</p>
                {error && <div style={{ padding:'12px 16px', background:'rgba(217,92,92,.1)', color:'#e07070', border:'1px solid rgba(217,92,92,.4)', borderRadius:8, fontSize:13, marginBottom:20 }}>{error}</div>}
                <form onSubmit={handleSubmit}>
                    {[
                        { id:'user', label:'Usuario',    type:'text',     val:username, set:setUsername },
                        { id:'pass', label:'Contraseña', type:'password', val:password, set:setPassword },
                    ].map(f => (
                        <div key={f.id} className="l-wrap" style={{ marginBottom: f.id==='pass' ? 28 : 20 }}>
                            <label style={{ color:focusField===f.id ? '#c9a84c' : t.muted }}>{f.label}</label>
                            <input type={f.type} value={f.val} required autoFocus={f.id==='user'}
                            ...
                            />
                            <div className="l-line" style={{ width:focusField===f.id?'100%':'0%' }} />
                        </div>
                    ))}
                    ...
                    <button type="button" disabled={loading} onClick={() => navigate('/guest')}
                            style={{ width:'100%', padding:'13px', background:'transparent', color:t.muted, border:'1px solid rgba(201,168,76,.3)', borderRadius:8, fontFamily:'Syne,sans-serif', fontSize:13, cursor:'pointer', marginBottom:24, transition:'all .3s' }}
                            onMouseEnter={e => { e.currentTarget.style.borderColor='#c9a84c'; e.currentTarget.style.color='#c9a84c'; e.currentTarget.style.background='rgba(201,168,76,.08)' }}
                            onMouseLeave={e => { e.currentTarget.style.borderColor='rgba(201,168,76,.3)'; e.currentTarget.style.color=t.muted; e.currentTarget.style.background='transparent' }}
                    >Entrar como invitado</button>
                </form>
                <p style={{ textAlign:'center', fontSize:13, color:t.muted }}>
                    ¿No tienes cuenta?{' '}<Link to="/register" style={{ color:'#c9a84c', textDecoration:'none', fontWeight:600 }}>Regístrate</Link>
                </p>
            </div>
        </div>
    )
